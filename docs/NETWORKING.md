...
```
/root/relay-server/    # hosted/server/ from repo
/root/viewer/          # hosted/viewer/ from repo (index.html)
...
```bash
# From local machine
scp -r hosted/server root@46.225.151.52:/root/relay-server
scp -r hosted/viewer root@46.225.151.52:/root/viewer
...
```bash
# Build for device
xcodebuild -project publishers/CameraAccess/CameraAccess.xcodeproj \
  -scheme CameraAccess \
...
| File | Purpose |
|------|---------|
| `hosted/server/src/server.ts` | Bun WebSocket relay server |
| `hosted/viewer/index.html` | Browser viewer with A/V sync |
| `publishers/CameraAccess/CameraAccess/ViewModels/StreamSessionViewModel.swift` | iOS view model, pipeline orchestration |
| `publishers/CameraAccess/CameraAccess/Pipeline/Stages/RelayStage.swift` | Video relay over WebSocket |
| `publishers/CameraAccess/CameraAccess/Pipeline/Stages/AudioStage.swift` | Mic capture, publishes to AudioEventBus |
| `publishers/CameraAccess/CameraAccess/Pipeline/Stages/AudioRelayStage.swift` | AudioEventBus subscriber, FRAU wire protocol |
| `publishers/CameraAccess/CameraAccess/Pipeline/AudioEventBus.swift` | AsyncStream pub/sub for audio packets |
| `publishers/CameraAccess/CameraAccess/Pipeline/AudioPacket.swift` | Transport-agnostic PCM data struct |
| `publishers/CameraAccess/CameraAccess/Pipeline/AudioTransport.swift` | Protocol for audio consumers |
| `publishers/CameraAccess/CameraAccess/Pipeline/FramePipelineManager.swift` | Frame dispatcher to stages |
