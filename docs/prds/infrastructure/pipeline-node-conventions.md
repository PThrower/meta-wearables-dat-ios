...
**Status:** Active — enforced rules from enhance node implementation
...
|---------|------|-------------|
| Server dispatch | `hosted/server/src/server.ts` (~line 1212) | Activation endpoint, categorized parallel dispatch |
| Server node definitions | `hosted/server/src/node-definitions.ts` | Node type registry, allowedTargets |
...
| Flow detection | `hosted/server/src/flow-detection.ts` | Union-Find for connected components |
| Frontend node defs | `hosted/web-platform/src/pages/workflow/node-defs.ts` | FALLBACK_PALETTE, edge validation |
| iOS pipeline manager | `publishers/CameraAccess/CameraAccess/Pipeline/FramePipelineManager.swift` | Frame dispatch, transform insertion |
| iOS transform stage | `publishers/CameraAccess/CameraAccess/Pipeline/Stages/FrameTransformStage.swift` | CIFilter chain, per-frame BGRA output |
| iOS enhance types | `publishers/CameraAccess/CameraAccess/Pipeline/EnhanceTypes.swift` | Filter config models |
| iOS vision stage | `publishers/CameraAccess/CameraAccess/Pipeline/Stages/VisionStage.swift` | VNDetectXxxRequest handlers |
| iOS ViewModel | `publishers/CameraAccess/CameraAccess/ViewModels/StreamSessionViewModel.swift` | WS message routing |

---

## Cross-References

- [PRD-003: On-Device AI Pipeline](ios/PRD-003-on-device-ai-pipeline.md) — Vision detection stage architecture
- [Relay Pipeline Architecture](relay/pipeline-architecture.md) — Server-side pipeline dispatch
- [FLOW-AND-THREADING](ios/FLOW-AND-THREADING.md) — iOS concurrency model
- [CLAUDE.md pipeline conventions](../../../.claude/rules/pipeline-conventions.md) — Same content as developer-facing rules
