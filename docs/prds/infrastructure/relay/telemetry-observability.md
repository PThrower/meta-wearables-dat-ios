...
This document was written against the codebase on `feat/telemetry-diagnostics` at commit `33ea802`. The `/stats` endpoint, `FrameTiming` interface, and `updateTiming()` EMA logic are in `hosted/server/src/server.ts`. Depends on:
