...
import {
  ...
  RotateCcw, Languages, Wrench, Settings
} from 'lucide-react';
...
const getStatusConfig = (status) => {
  switch (status) {
    case 'active':
      return {
        badge: 'bg-[#E0FF00]/15 text-[#E0FF00] border-[#E0FF00]/30',
        dot: 'bg-[#E0FF00] animate-pulse',
        label: 'Live',
        icon: Radio,
      };
    case 'paused':
...
function TranscriptPanel({ transcript, isLive }) {
  ...
  return (
    <div ref={scrollRef} className="space-y-1.5 max-h-80 overflow-y-auto pr-2">
      ...
      {isLive && (
        <div className="flex items-center gap-2 text-xs text-[#E0FF00] pt-1">
          <span className="h-1.5 w-1.5 rounded-full bg-[#E0FF00] animate-pulse" />
          Listening...
        </div>
      )}
    </div>
          ...
          <div className="space-y-1">
            {analysis.action_items.map((item, i) => (
              <div key={i} className="flex items-start gap-2 text-xs">
                <ArrowRight className="h-3 w-3 text-[#E0FF00] mt-0.5 flex-shrink-0" />
            ...
            {analysis.safety_flags.map((flag, i) => (
              <div key={i} className="flex items-start gap-2 text-xs">
                <Shield className={`h-3 w-3 mt-0.5 flex-shrink-0 ${
                  ...
                  flag.severity === 'warning' ? 'text-amber-400' : 'text-[#E0FF00]'
                }`} />
              ...
              <div className="h-2 w-20 bg-muted/50 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${
                    analysis.customer_sentiment.score >= 0.7 ? 'bg-[#E0FF00]' :
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(analysis.compliance_check).map(([key, value]) => (
              <div key={key} className={`flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] ${
                value ? 'bg-[#E0FF00]/10 text-[#E0FF00]' : 'bg-red-500/10 text-red-400'
              }`}>
  ...
  if (activeSession && liveTokenSource) {
    return (
      <div className="min-h-screen bg-background text-foreground" data-testid="live-stream-page">
        <Header />
        <main className="h-[calc(100vh-64px)] grid grid-cols-1 lg:grid-cols-[30%_40%_30%] gap-3 p-3">
          {/* CENTER — Video Feed, full height, no borders (order-2 on desktop) */}
          <div className="relative min-h-0 lg:order-2 overflow-hidden">
            <Suspense fallback={
              <div className="absolute inset-0 flex items-center justify-center bg-black">
                <Loader2 className="h-6 w-6 animate-spin text-[#E0FF00]" />
              </div>
            }>
              <LiveKitViewer
                ...
                className="absolute inset-0 flex flex-col [&_video]:object-cover [&_video]:w-full [&_video]:h-full"
              />
            </Suspense>
          </div>
          ...
          <div className="flex flex-col min-h-0 lg:order-1">
            {/* Transcript area */}
            <div className="flex-1 min-h-0 bg-white/5 border border-white/10 rounded-lg flex flex-col overflow-hidden">
            ...
            <div className="mt-3 px-1">
              <div className="text-[9px] text-[#E0FF00]/50 font-semibold uppercase tracking-widest mb-2">Skills</div>
...
Be encouraging, patient, and interactive. Ask questions, don't just lecture. Use objects in their environment as examples when possible.`;
                  ...
                  className="w-full text-left bg-white/5 border border-white/10 rounded p-2 hover:border-[#E0FF00]/30 transition-colors"
                >
...
- If the situation seems urgent like chest pain, difficulty breathing, or severe bleeding, immediately say: "This sounds like it could be serious. Please call emergency services or go to the nearest emergency room right away."`;
          ...
          <div className="flex flex-col min-h-0 overflow-y-auto lg:order-3">
            <div className="bg-white/5 border border-white/10 rounded-lg flex flex-col overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10 flex-shrink-0">
                <Settings className="h-3.5 w-3.5 text-[#E0FF00]" strokeWidth={1.5} />
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-[9px] text-[#E0FF00]/50 font-semibold uppercase tracking-widest">Apply Extracted Skill</div>
                      {activeSkillId && (
                        <span className="text-[9px] text-[#E0FF00] bg-[#E0FF00]/10 px-1.5 py-0.5 rounded">Active</span>
                      )}
                    </div>
                    {extractedSkills.length === 0 ? (
                      <p className="text-[10px] text-white/30">
                        No skills yet. Extract from a recording on the{' '}
                        <button onClick={() => navigate('/feeds')} className="text-[#E0FF00]/70 hover:text-[#E0FF00] underline">Feeds page</button>.
                      </p>
                    ) : (
                          ...
                          <button
                            ...
                            className={`w-full text-left rounded p-2 border transition-colors ${
                              activeSkillId === skill.id
                                ? 'bg-[#E0FF00]/10 border-[#E0FF00]/40'
                                : 'bg-white/5 border-white/10 hover:border-[#E0FF00]/30'
                            }`}
                  ...
                  <div>
                    <div className="text-[9px] text-[#E0FF00]/50 font-semibold uppercase tracking-widest mb-2">Update Agent Instructions</div>
                    <textarea
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-xs text-white placeholder:text-white/15 focus:outline-none focus:border-[#E0FF00]/30 min-h-[70px] resize-y"
                    />
                    <Button
                      variant="outline"
                      size="sm"
                      ...
                      className="w-full mt-2 border-[#E0FF00]/30 text-[#E0FF00] hover:bg-[#E0FF00]/10 text-[10px]"
                    >
                      Update Instructions
                    </Button>
                  ...
                  <div>
                    <div className="text-[9px] text-[#E0FF00]/50 font-semibold uppercase tracking-widest mb-2">Upload Knowledge</div>
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="text-[10px] text-[#E0FF00] border border-[#E0FF00]/30 rounded px-3 py-1 hover:bg-[#E0FF00]/10"
                          >Upload</button>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="mt-1 text-[10px] text-[#E0FF00] border border-[#E0FF00]/30 rounded px-3 py-1 hover:bg-[#E0FF00]/10 w-full"
                  ...
                  <div>
                    <div className="text-[9px] text-[#E0FF00]/50 font-semibold uppercase tracking-widest mb-2">Active Tools</div>
                    <div className="space-y-1">
                      {(agentTools || [
                        ...
                        <div key={tool.name} className="flex items-center gap-2 text-[10px]">
                          <Wrench className="h-3 w-3 text-[#E0FF00]/40" strokeWidth={1.5} />
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="text-[9px] text-white/30 hover:text-[#E0FF00] transition-colors"
                      >Refresh</button>
                    </div>
                    {agentConfig ? (
                      <div className="space-y-3">
    ...
    <div className="min-h-screen bg-background text-foreground" data-testid="live-stream-page">
      ...
      <div className="fixed inset-0 -z-10 pointer-events-none">
        <div
          ...
          style={{
            background: 'radial-gradient(60% 50% at 80% 10%, rgba(224,255,0,0.08) 0%, transparent 50%)',
          }}
        />
      </div>
      ...
      <main className="px-4 sm:px-6 py-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-2xl font-semibold tracking-tight" data-testid="live-stream-title">
                  <span className="text-foreground">Live</span>{' '}
                  <span className="text-[#E0FF00]">Streams</span>
                </h1>
                {sessions.length > 0 && (
                  <Badge variant="outline" className="text-xs bg-[#E0FF00]/10 text-[#E0FF00] border-[#E0FF00]/30">
        ...
        {loading && sessions.length === 0 ? (
          ...
          <div className="text-center py-20">
            <Glasses className="h-12 w-12 text-muted-foreground/50 mx-auto mb-4" strokeWidth={1} />
            <h3 className="text-lg font-medium text-muted-foreground mb-2">No active streams right now</h3>
          ...
          <div className="space-y-3">
            {sessions.map((session, index) => {
              ...
              return (
                ...
                >
                  <div
                    className="clip-chamfer bg-card border border-border hover:border-[#E0FF00]/30 transition-colors cursor-pointer"
                    onClick={() => {
                    ...
                    <div className="p-4 flex items-center gap-4">
                      <div className="flex-shrink-0">
                        <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${
                          session.status === 'active' ? 'bg-[#E0FF00]/15' : 'bg-muted/50'
                        }`}>
                          <StatusIcon className={`h-5 w-5 ${
                            session.status === 'active' ? 'text-[#E0FF00]' : 'text-muted-foreground'
                          }`} strokeWidth={1.5} />
                        </div>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className="text-sm font-medium text-foreground truncate hover:text-[#E0FF00] transition-colors cursor-pointer"
                            onClick={(e) => {
                              if (session.device_id) {
                                e.stopPropagation();
                                navigate(`/devices/${session.device_id}`);
                              }
                            }}
                          >
                            {session.devices?.name || session.device_name || 'Unknown Device'}
                          </span>
                          {session.devices?.model && (
                            <span className="text-[10px] text-white/30">{session.devices.model}</span>
                          )}
                          <Badge
                            variant="outline"
                            className={`text-[10px] px-1.5 py-0 ${statusConfig.badge}`}
                          >
                            <span className={`h-1.5 w-1.5 rounded-full mr-1 ${statusConfig.dot}`} />
                            {statusConfig.label}
                          </Badge>
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          ...
                          {session.devices?.device_id && (
                            <span className="font-mono text-[10px] text-white/30">{session.devices.device_id}</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex-shrink-0 flex items-center gap-3">
                        {session.status === 'active' && (
                          <Button
                            variant="default"
                            size="sm"
                            className="bg-[#E0FF00] hover:bg-[#E0FF00]/90 text-black text-xs"
                          >
