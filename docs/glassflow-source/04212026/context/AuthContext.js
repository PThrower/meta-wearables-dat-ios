...
import { setClerkSupabaseTokenGetter } from '@/lib/clerkSupabaseBridge';
...
const ClerkAuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (isSignedIn && clerkUser) {
      ...
      // Register Clerk's token getter so the Supabase client can mint
      // `supabase`-template JWTs on demand (used for org-scoped RLS).
      setClerkSupabaseTokenGetter(getToken);

      // Set the Clerk JWT as the API bearer token (FastAPI backend side).
      getToken().then((token) => {
      ...
      setUser(null);
      setAuthToken(null);
      setClerkSupabaseTokenGetter(null);
      setTokenReady(true);
    }
