...
|---------|-------|-------------|
| `relay/crate` | None | `FrameRelay` is already stateless-per-instance. Instantiate one per session instead of one globally. |
| `relay/server` | Major | Replace `let publisher` + global `viewers` with `sessions` map. Route by `session` query param. Add `/sessions` and `/session/:id` endpoints. Per-session stale-cleanup. |
| `relay/viewer` | Additive | Accept `?session=` param. Add session-chooser UI for directory page. Auto-reconnect stays, scoped to session. |
