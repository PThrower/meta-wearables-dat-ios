...
Location: `samples/CameraAccess/CameraAccess/Pipeline/`
...
The Bun server (`relay/server/src/server.ts`) instantiates one `FrameRelay` **per viewer** with the viewer's chosen quality preset:
