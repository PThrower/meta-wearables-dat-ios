...
export class GuidanceOrchestrator {
  ...
  async activateWithConfig(sessionId: string, app: AppDefinition): Promise<void> {
    ...
    const input: InputConfig = (app.config.input as InputConfig) ?? { camera: "glasses", audio: "phone-mic", codec: "jpeg", visionFps: app.config.visionFps ?? 1 };
  ...
  /** Get the input config for an active session (used to configure publisher sources) */
