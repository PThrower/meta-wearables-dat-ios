...
export const NODE_DEFINITIONS: NodeDefinition[] = [
  ...
  },
  {
    ...
    configSchema: [
      ...
      { kind: "checkbox", key: "reconciliationEnabled", label: "Tracklet Reconciliation (ID persistence across long occlusions)" },
      { kind: "number", key: "reconciliationMaxGap", label: "Reconciliation Max Gap (frames, ~2s default)", min: 1, max: 300, step: 1 },
      { kind: "range", key: "reconciliationThreshold", label: "Reconciliation Appearance Threshold (lower=stricter)", min: 0.1, max: 1.0, step: 0.05 },
      { kind: "range", key: "reconciliationSpatialWeight", label: "Reconciliation Spatial Weight (0=appearance only, 1=spatial only)", min: 0, max: 1, step: 0.05 },
      // Visual analytics — always computed, no toggle needed (zero overhead when no zones defined)
    ],
  ...
  },
  {
    type: "overlays",
    label: "Overlays",
    subtitle: "bbox + ${transcription} + tool sizing",
    color: { fill: "#3d2000", header: "#f97316", stroke: "#f97316" },
    allowedTargets: [],
    role: "sink",
    activationMode: null,
    ...
    configSchema: [
      ...
      { kind: "checkbox", key: "showToolDimensions", label: "Show tool dimensions (mm)" },
      { kind: "checkbox", key: "showToolSize", label: "Show suggested tool size" },
      { kind: "checkbox", key: "showToolConfidence", label: "Show measurement confidence" },
    ],
    defaultConfig: { bbox: true, transcription: false, showToolDimensions: true, showToolSize: true, showToolConfidence: true },
...

  {
    ...
    configSchema: [
      ...
      { kind: "text", key: "objectTypeId", label: "Object Type ID (TrackedPerson)", placeholder: "TrackedPerson" },
      { kind: "select", key: "operation", label: "Operation", options: [
        ...
        { value: "tracking_sync", label: "Tracking Sync (create/update objects from tracker)" },
      ]},
      { kind: "section", label: "Tracking Sync — Object Type IDs", fields: [
        { kind: "text", key: "transitionObjectTypeId", label: "ZoneTransition Type ID", placeholder: "ZoneTransition (defaults to Object Type ID above)" },
        { kind: "text", key: "zoneObjectTypeId", label: "MonitoringZone Type ID", placeholder: "MonitoringZone (defaults to Object Type ID above)" },
      ]},
...

  {
    ...
    allowedTargets: ["s2s-live", "s2s-rest", "s2s-e4b", "jepa-vision", "local-tts", "overlays", "gate-mahalanobis", "gate-iou", "gate-bhattacharyya", "gate-reid", "cost-iou", "tracking-ocsort", "tracking-heatmap", "palantir-ontology", "palantir-aip", "palantir-dataset", "palantir-llm", "palantir-action", "yolo-detect", "yolo-segment", "yolo-pose", "<sink>", "<trigger>"],
    role: "processor",
    activationMode: "yolo",
    binding: null,
    defaultModel: null,
    ...
    defaultConfig: { modelId: "yolo11n", modelUrl: "https://github.com/ultralytics/yolo-ios-app/releases/download/v8.3.0/yolo11n.mlpackage.zip", confidence: 0.25, iouThreshold: 0.45, targetFPS: 10, maxDetections: 100, inputSize: 640, smoothingAlpha: 0.3 },
  ...
  },
  {
    ...
    defaultConfig: { modelId: "yolo11n-seg", modelUrl: "https://github.com/ultralytics/yolo-ios-app/releases/download/v8.3.0/yolo11n-seg.mlpackage.zip", confidence: 0.25, iouThreshold: 0.45, targetFPS: 10, maxDetections: 100, inputSize: 640, smoothingAlpha: 0.3 },
  ...
  },
  {
    ...
    defaultConfig: { modelId: "yolo11n-pose", modelUrl: "https://github.com/ultralytics/yolo-ios-app/releases/download/v8.3.0/yolo11n-pose.mlpackage.zip", confidence: 0.25, iouThreshold: 0.45, targetFPS: 10, maxDetections: 100, inputSize: 640, smoothingAlpha: 0.3 },
