...
export const NODE_DEFINITIONS: NodeDefinition[] = [
  ...
  // --- Vision processor nodes (on-device Apple Vision framework) ---
  {
    ...
    defaultConfig: { confidence: 0.5, maxFaces: 0, targetFPS: 5 },
  ...
  },
  {
    ...
    defaultConfig: { language: "en-US", confidence: 0.5, targetFPS: 2 },
    defaultLabel: "OCR",
    runtime: ["mobile"],
  },
  {
    ...
    defaultConfig: { confidence: 0.3, maxLabels: 5, targetFPS: 5 },
  ...
  // --- Sensor nodes (on-device sound classification + location) ---
  {
    ...
    defaultConfig: { windowDuration: 1.5, overlapFactor: 0.5, confidence: 0.3, maxLabels: 5, targetLabels: "" },
  ...
  },
  {
    ...
    configSchema: [
      { kind: "text", key: "label", label: "Label" },
      { kind: "select", key: "mode", label: "Mode", options: [
        { value: "continuous", label: "Continuous GPS updates" },
        { value: "significant", label: "Significant changes only (~500m, battery efficient)" },
        { value: "visits", label: "Visit detection (arrive/stay/leave)" },
        { value: "geofence", label: "Geofence monitoring (enter/exit regions)" },
      ]},
