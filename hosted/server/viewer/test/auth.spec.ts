...
/** Create a fake JWT token with the given email and expiration */
...
/**
 * Simulate a full Google login:
 * 1. Set token in localStorage
 * 2. Hide login overlay (as handleGoogleLogin would)
 * 3. Dispatch auth:login event
 */
async function simulateLogin(page: Page, email = "test@example.com"): Promise<void> {
  await page.evaluate((token) => {
    localStorage.setItem("relay_token", token);
    // Hide overlay (normally done by handleGoogleLogin)
...
test.describe("Auth - noAuth mode", () => {
  ...
  test("no user info shown when noAuth=true", async ({ page }) => {
    ...
    await expect(userInfo).toBeEmpty();
  });
});
...
test.describe("Auth - gated mode", () => {
  ...
  test("gallery fetches data after login event", async ({ page }) => {
    ...
    // Wait for gallery to be fetched and cards rendered
  ...
  test("expired token triggers re-auth on boot", async ({ page }) => {
    ...
    // Use addInitScript to set the token before page JS runs
