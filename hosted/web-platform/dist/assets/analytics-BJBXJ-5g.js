import{f as V,b as H,k as W,h as O,e as U,l as K}from"./main-Bi2az7yX.js";import"./modulepreload-polyfill-B5Qt9EMX.js";const Y={width:600,height:200,color:"#4ade80",fillOpacity:.1,strokeWidth:2,showDots:!0,showGrid:!0,gridLines:4,formatLabel:t=>t,formatValue:t=>String(t)};function Z(t,e,s={}){const l={...Y,...s},{width:o,height:r,color:a,fillOpacity:c,strokeWidth:f,showDots:m,showGrid:v,gridLines:g}=l;if(e.length===0){t.innerHTML='<p class="empty-state">No data</p>';return}const h={top:10,right:20,bottom:30,left:40},k=o-h.left-h.right,S=r-h.top-h.bottom,L=e.map(n=>n.value),$=Math.min(...L,0),A=Math.max(...L,1),M=A-$||1,d=n=>h.left+n/Math.max(e.length-1,1)*k,u=n=>h.top+S-(n-$)/M*S;let b="";if(v)for(let n=0;n<=g;n++){const i=h.top+n/g*S,p=A-n/g*M;b+=`<line x1="${h.left}" y1="${i}" x2="${o-h.right}" y2="${i}" stroke="rgba(255,255,255,0.06)" />`,b+=`<text x="${h.left-6}" y="${i+3}" text-anchor="end" fill="rgba(255,255,255,0.3)" font-size="10" font-family="SF Mono, Menlo, monospace">${l.formatValue(p)}</text>`}const E=Math.max(1,Math.floor(e.length/8));let T="";for(let n=0;n<e.length;n++)(n%E===0||n===e.length-1)&&(T+=`<text x="${d(n)}" y="${r-6}" text-anchor="middle" fill="rgba(255,255,255,0.3)" font-size="10" font-family="SF Mono, Menlo, monospace">${l.formatLabel(e[n].label)}</text>`);const N=e.map((n,i)=>`${i===0?"M":"L"}${d(i)},${u(n.value)}`).join(" "),j=N+` L${d(e.length-1)},${u($)} L${d(0)},${u($)} Z`;let q="";m&&(q=e.map((n,i)=>`<circle cx="${d(i)}" cy="${u(n.value)}" r="3" fill="${a}" stroke="#0a0a0a" stroke-width="1.5" class="chart-dot" data-value="${n.value}" data-label="${n.label}" />`).join(""));const z=`
  ...
  `;t.innerHTML=z;const C=t.querySelector("svg");if(C&&m){const n=C.querySelectorAll(".chart-dot");let i=null;n.forEach(p=>{p.addEventListener("mouseenter",()=>{const I=p.dataset.value,F=p.dataset.label;if(!I||!F)return;const G=Number(p.getAttribute("cx")),R=Number(p.getAttribute("cy"));i=document.createElementNS("http://www.w3.org/2000/svg","text"),i.setAttribute("x",String(G)),i.setAttribute("y",String(R-10)),i.setAttribute("text-anchor","middle"),i.setAttribute("fill","#fff"),i.setAttribute("font-size","11"),i.setAttribute("font-family","SF Mono, Menlo, monospace"),i.textContent=`${l.formatLabel(F)}: ${l.formatValue(Number(I))}`,C.appendChild(i)}),p.addEventListener("mouseleave",()=>{i&&(i.remove(),i=null)})})}}const J={width:240,height:240,radius:80,thickness:24,centerLabel:"",centerValue:""};function Q(t,e,s={}){const l={...J,...s},{width:o,height:r,radius:a,thickness:c,centerLabel:f,centerValue:m}=l;if(e.length===0){t.innerHTML='<p class="empty-state">No data</p>';return}const v=o/2,g=r/2,h=e.reduce((d,u)=>d+u.value,0)||1,k=2*Math.PI*a;let S="",L="",$=0;for(const d of e){const u=d.value/h,b=u*k,E=k-b;S+=`<circle
      cx="${v}" cy="${g}" r="${a}"
      fill="none"
      stroke="${d.color}"
      stroke-width="${c}"
