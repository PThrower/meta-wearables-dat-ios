 ...
 *
 * This is a frontend copy of the server-side flow-detection.ts.
...
/**
 * Build a default FlowExecutionConfig from detected flows.
 * Returns null when only one flow (no config needed).
 */
export function buildDefaultFlowConfig(flows: DetectedFlow[]): FlowExecutionConfig | null {
  if (flows.length <= 1) return null;
  return { mode: "parallel", flowOrder: flows.map(f => f.flowId) };
}
