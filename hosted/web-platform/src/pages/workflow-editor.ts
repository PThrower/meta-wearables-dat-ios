...
async function renderEditor(isNew: boolean): Promise<void> {
  ...
  if (isNew) {
    ...
    _workflow = {
      ...
      ownerId: null,
      nodes: [
        { id: `n_src_${now}`, type: "stream-input", label: "Input", config: { video: true, phoneMic: true, glassesMic: false, gestures: true, visionFps: 1 }, positionX: 50, positionY: 260 },
        ...
        { id: `n_ann_${now}`, type: "text", label: "Announcement", config: { text: "Hello! I'm your smart glasses assistant. I'm here to help." }, positionX: 50, positionY: 460 },
        { id: `n_tts_${now}`, type: "local-tts", label: "Speak", config: {}, positionX: 320, positionY: 460 },
        { id: `n_psp_${now}`, type: "phone-speaker", label: "Phone Speaker", config: {}, positionX: 600, positionY: 400 },
        { id: `n_gsp_${now}`, type: "glasses-speaker", label: "Glasses Speaker", config: { profile: "hfp" }, positionX: 600, positionY: 500 },
      ...
      edges: [
        ...
        { id: `e_3_${now}`, sourceNodeId: `n_ai_${now}`, targetNodeId: `n_ovl_${now}` },
        { id: `e_4_${now}`, sourceNodeId: `n_ann_${now}`, targetNodeId: `n_tts_${now}` },
        { id: `e_5_${now}`, sourceNodeId: `n_tts_${now}`, targetNodeId: `n_psp_${now}` },
        { id: `e_6_${now}`, sourceNodeId: `n_tts_${now}`, targetNodeId: `n_gsp_${now}` },
      ],
