...
import type { WorkflowDetail, NodeDefinition, ConfigFieldSchema } from "./types.js";
...
function fmtConfigField(f: ConfigFieldSchema, indent = "  "): string[] {
  const lines: string[] = [];
  switch (f.kind) {
    case "text":
    ...
    case "select":
      ...
      for (const o of f.options) lines.push(`${indent}    ${o.value} = ${o.label}`);
      break;
    case "range":
    ...
    case "checkbox-group":
      ...
      for (const sub of f.fields) lines.push(`${indent}    ${sub.key} = ${sub.label}`);
      break;
    case "geofence-map":
    ...
    case "section":
      ...
      for (const sub of f.fields) lines.push(...fmtConfigField(sub, indent + "  "));
      break;
    case "conditional":
...
Returns the full WorkflowSettings object with field descriptions.`,
    ...
    async ({ workflow_id }) => {
      try {
        const w = await api.getWorkflow(workflow_id);
        if (!w.settings) return { content: [{ type: "text", text: `Workflow "${w.name}" has no settings configured (using defaults).` }] };
        const s = w.settings;
        const lines = [
          `# Settings — ${w.name}`,
          "",
...
- targetDeviceId: specific device UUID to target (null = any device)`,
    ...
    async ({ workflow_id, ...params }) => {
      try {
        ...
        if (params.on_disconnect !== undefined) current.onDisconnect = params.on_disconnect;
        if (params.on_reconnect !== undefined) current.onReconnect = params.on_reconnect;
        if (params.auto_deactivate_min !== undefined) current.autoDeactivateMin = params.auto_deactivate_min;
        if (params.max_session_duration !== undefined) current.maxSessionDuration = params.max_session_duration;
        if (params.recording_enabled !== undefined) current.recordingEnabled = params.recording_enabled;
        if (params.viewer_access !== undefined) current.viewerAccess = params.viewer_access;
        if (params.telemetry_interval_sec !== undefined) current.telemetryIntervalSec = params.telemetry_interval_sec;
        if (params.wake_on_activate !== undefined) current.wakeOnActivate = params.wake_on_activate;
        if (params.auto_start_stream !== undefined) current.autoStartStream = params.auto_start_stream;
        if (params.target_device_id !== undefined) current.targetDeviceId = params.target_device_id;
...
Returns the full FlowExecutionConfig with human-readable trigger details.`,
    ...
    async ({ workflow_id }) => {
      try {
        const w = await api.getWorkflow(workflow_id);
        if (!w.flowConfig) return { content: [{ type: "text", text: `Workflow "${w.name}" has no flow config (default: parallel).` }] };
        ...
        if (fc.flowTriggers && Object.keys(fc.flowTriggers).length) {
          ...
          for (const [flowId, trigger] of Object.entries(fc.flowTriggers)) {
            ...
            if (trigger.sourceFlowId) lines.push(`Source flow: ${trigger.sourceFlowId}`);
            if (trigger.condition) {
              const c = trigger.condition;
              lines.push(`Condition: ${c.field} ${c.operator} ${c.value} (from flow ${c.sourceFlowId})`);
            }
            if (trigger.intervalSec) lines.push(`Interval: ${trigger.intervalSec}s`);
            if (trigger.jepaEvent) lines.push(`JEPA event: ${trigger.jepaEvent} (threshold: ${trigger.jepaConfidenceThreshold ?? "default"})`);
          }
        }
        return { content: [{ type: "text", text: lines.join("\n") }] };
      } catch (err) { return handleErr(err); }
...
Pass the full flow_config as a JSON string.`,
    ...
    async ({ workflow_id, mode, flow_order, flow_triggers }) => {
      try {
        const flowConfig: Record<string, unknown> = { mode, flowOrder: flow_order ? JSON.parse(flow_order) : [] };
        if (flow_triggers) flowConfig.flowTriggers = JSON.parse(flow_triggers);

        const updated = await api.updateWorkflow(workflow_id, {
          flowConfig: JSON.stringify(flowConfig),
        });
        return { content: [{ type: "text", text: `Updated flow config for "${updated.name}": mode=${mode}, ${flowConfig.flowOrder ? `${(flowConfig.flowOrder as string[]).length} flows in order` : "no order"}, ${flow_triggers ? `${Object.keys(JSON.parse(flow_triggers)).length} triggers` : "no triggers"}` }] };
      } catch (err) { return handleErr(err); }
    },
  );
...
Pass the node config as a JSON string. Returns a list of issues (or "valid" if none).`,
    ...
    async ({ node_type, config }) => {
      try {
        ...
        const def = defs.find((d) => d.type === node_type);
        if (!def) return { content: [{ type: "text", text: `Unknown node type: ${node_type}` }] };
        ...
        function collectFields(fields: Array<Record<string, unknown>>): void {
          for (const f of fields) {
            if (f.kind === "section" || f.kind === "conditional") {
              collectFields((f.fields ?? []) as Array<Record<string, unknown>>);
            } else if (f.key) {
              schemaKeys.add(f.key as string);
              const val = parsed[f.key as string];
              if (val === undefined) return; // missing keys handled below

              switch (f.kind) {
                case "range":
                  if (typeof val === "number") {
                    if (val < (f.min as number)) issues.push(`${f.key}: ${val} below minimum ${f.min}`);
                    if (val > (f.max as number)) issues.push(`${f.key}: ${val} above maximum ${f.max}`);
                  } else {
                ...
                case "select":
                  if (typeof val === "string") {
                    const validValues = ((f.options ?? []) as Array<{ value: string }>).map((o) => o.value);
                    if (!validValues.includes(val)) issues.push(`${f.key}: "${val}" not in options [${validValues.join(", ")}]`);
                  } else {
                ...
                case "number":
                  if (typeof val === "number") {
                    if (f.min !== undefined && val < (f.min as number)) issues.push(`${f.key}: ${val} below minimum ${f.min}`);
                    if (f.max !== undefined && val > (f.max as number)) issues.push(`${f.key}: ${val} above maximum ${f.max}`);
                  } else {
                    issues.push(`${f.key}: expected number, got ${typeof val}`);
                  }
                  break;
        ...
        collectFields(def.configSchema as unknown as Array<Record<string, unknown>>);
      ...
      try {
        ...
        if (node_type.startsWith("vision-")) {
          if (uc.includes("fast") || uc.includes("sport") || uc.includes("motion")) {
            config.targetFPS = 10; config.smoothingAlpha = 0.5;
          ...
          if (uc.includes("accurate") || uc.includes("precise") || uc.includes("careful")) {
            config.confidence = 0.7; config.smoothingAlpha = 0.2; config.targetFPS = 3;
        ...
        if (node_type === "tracking-ocsort") {
          if (uc.includes("crowd") || uc.includes("busy") || uc.includes("many")) {
            config.maxTracks = 50; config.maxAge = 60; config.minHits = 5;
            adjustments.push("maxTracks=50, maxAge=60, minHits=5 (crowd scenario)");
          }
          if (uc.includes("fast") || uc.includes("sport")) {
            config.targetFPS = 15; config.iouThreshold = 0.2; config.deltaT = 5; config.inertia = 0.4;
          ...
          if (uc.includes("long") || uc.includes("persistent") || uc.includes("occlusion")) {
            config.reconciliationEnabled = true; config.reconciliationMaxGap = 120; config.maxAge = 60;
        ...
        if (node_type.startsWith("s2s-")) {
          if (uc.includes("fast") || uc.includes("realtime") || uc.includes("responsive")) {
            config.visionFps = 2; config.analysisIntervalSec = 1; config.temperature = 0.3;
        ...
        if (node_type === "enhance-night-mode") {
          if (uc.includes("extreme") || uc.includes("very dark")) {
            config.brightness = 0.3; config.gamma = 0.6; config.highlightAmount = 2.5;
        ...
        if (node_type === "enhance-brightness") {
          if (uc.includes("low-light") || uc.includes("dark")) {
            config.brightness = 0.3; config.contrast = 1.3;
        ...
        if (node_type === "sensor-location") {
          if (uc.includes("precise") || uc.includes("navigation")) {
            config.accuracy = "best"; config.minDistance = 1; config.updateIntervalSec = 1;
          ...
          if (uc.includes("battery") || uc.includes("efficient")) {
            config.accuracy = "hundredMeters"; config.minDistance = 100; config.updateIntervalSec = 60;
