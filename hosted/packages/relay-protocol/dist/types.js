...
export function updateTiming(t, sequence, timestampMs) {
    const now = Date.now();
    if (t.lastReceivedAt > 0) {
        ...
        if (interval > 0) {
            ...
            if (interval < t.minIntervalMs)
                t.minIntervalMs = interval;
            if (interval > t.maxIntervalMs)
                t.maxIntervalMs = interval;
        }
