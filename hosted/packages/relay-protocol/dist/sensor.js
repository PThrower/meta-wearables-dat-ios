...
export function isSensorFrame(buf) {
    return buf.length >= 4 && buf[0] === FRSE_MAGIC[0] && buf[1] === FRSE_MAGIC[1] && buf[2] === FRSE_MAGIC[2] && buf[3] === FRSE_MAGIC[3];
}
export function parseSensorHeader(buf) {
    if (buf.length < SENSOR_HEADER_SIZE)
        return null;
    if (buf[0] !== FRSE_MAGIC[0] || buf[1] !== FRSE_MAGIC[1] || buf[2] !== FRSE_MAGIC[2] || buf[3] !== FRSE_MAGIC[3])
        return null;
    const version = buf[4];
