 ...
 */
import { FRLY_MAGIC, HEADER_SIZE, PROTOCOL_VERSION, CRC_OFFSET, HEADER_BEFORE_CRC, VIDEO_CODEC_H264, H264_FLAG_KEYFRAME, H264_FLAG_SPSPPS } from "./constants.js";
import { crc16 } from "./crc16.js";
export function isVideoFrame(buf) {
    return buf.length >= 4 && buf[0] === FRLY_MAGIC[0] && buf[1] === FRLY_MAGIC[1] && buf[2] === FRLY_MAGIC[2] && buf[3] === FRLY_MAGIC[3];
}
export function parseVideoHeader(buf) {
    if (buf.length < HEADER_SIZE)
        return null;
    if (buf[0] !== FRLY_MAGIC[0] || buf[1] !== FRLY_MAGIC[1] || buf[2] !== FRLY_MAGIC[2] || buf[3] !== FRLY_MAGIC[3])
        return null;
    const version = buf[4];
 ...
 */
export function buildVideoFrame(sequence, width, height, quality, timestampMs, jpegPayload) {
